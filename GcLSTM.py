...
# LSTM 网络
class GcLSTMNetwork(nn.Module):
    def __init__(self, input_dim, hidden_dim, adj, device):
        super(GcLSTMNetwork, self).__init__()
        self.device = device
        self.cell = GcLSTMCell(input_dim, hidden_dim, adj, device)  # 使用LSTM替代GRU
        self.dense = nn.Linear(hidden_dim, 1)
        self.dropout = nn.Dropout(0.3)  # 添加Dropout层

    def forward(self, x):
        batch_size, seq_len, _ = x.size()
        h = torch.zeros(batch_size, self.cell.hidden_dim).to(self.device)  # 初始化隐藏状态
        c = torch.zeros(batch_size, self.cell.hidden_dim).to(self.device)  # 初始化细胞状态
        for t in range(seq_len):
            h, c = self.cell(x[:, t, :], h, c)
        h = self.dropout(h)  # 在全连接层前应用Dropout
        output = self.dense(h)
        return output
...
def train_and_evaluate(data_addr, adj_addr):
    ...
    model = GcLSTMNetwork(input_dim, hidden_dim, adj, device).to(device)
